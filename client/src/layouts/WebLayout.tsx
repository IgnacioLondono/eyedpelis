      ...
      <aside className="hidden md:flex flex-col w-64 bg-surface-card border-r border-purple-500/10 fixed h-full z-50 animate-slide-in-left">
        ...
        <nav className="flex-1 px-3 space-y-1">
          {navItems.map(({ to, icon: Icon, label }, i) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `nav-link flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium animate-fade-in-up focus-visible:outline-none ${
                  isActive
        ...
        {enabled && username && (
          <div className="p-4 border-t border-surface-border">
            ...
            <button
              onClick={logout}
              className="flex items-center gap-2 text-gray-400 hover:text-white w-full px-4 py-2 rounded-lg hover:bg-surface-hover transition-colors focus-visible:outline-none text-sm"
            >
