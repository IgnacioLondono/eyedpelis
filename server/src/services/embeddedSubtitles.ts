...
const BITMAP_CODECS = new Set(['hdmv_pgs_subtitle', 'dvd_subtitle', 'dvdsub', 'pgssub']);
  ...
  try {
    ...
    for (const stream of data.streams || []) {
      ...
      if (BITMAP_CODECS.has(codec)) {
        tracks.push({
          path: videoPath,
          label: `${tagTitle || baseLabel} (imagen)`,
          language: lang,
          format: 'ass',
          embedded: true,
          bitmap: true,
...
function runFfmpegExtract(args: string[]): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    ffmpeg.on('close', (code) => {
      if (code !== 0) {
        reject(new Error(errors.join('').trim() || `ffmpeg exit ${code}`));
        return;
      }
...
async function extractEmbeddedToVtt(videoPath: string, subIndex: number, streamIndex?: number): Promise<string> {
  const baseArgs = ['-hide_banner', '-loglevel', 'error', '-i', videoPath];
  const outArgs = ['-c:s', 'webvtt', '-f', 'webvtt', 'pipe:1'];

  const attempts: string[][] = [];
  if (streamIndex != null) {
    attempts.push([...baseArgs, '-map', `0:${streamIndex}`, ...outArgs]);
  }
  attempts.push([...baseArgs, '-map', `0:s:${subIndex}`, ...outArgs]);
  if (streamIndex != null) {
    attempts.push([...baseArgs, '-map', `0:s:${subIndex}`, ...outArgs]);
  }
  ...
  for (const args of attempts) {
    try {
      return await runFfmpegExtract(args);
    } catch (err) {
      lastErr = err instanceof Error ? err : new Error(String(err));
    }
  }
  throw lastErr || new Error('No se pudo extraer el subtítulo');
}
